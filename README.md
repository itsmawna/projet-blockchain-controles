...
flowchart TD
  ...
  C --> C1[git clone puis cd projet]

  C1 --> D[Installation dépendances]
  D --> D1[npm install - racine]
  D --> D2[frontend - npm install]
  D --> D3[upload-server - npm install express cors multer]

  D3 --> E[Configuration Hardhat]
  E --> E1[Créer hardhat.config.js]
  E1 --> E2[Solidity 0.8.19]
  E2 --> E3[Réseau localhost - chainId 31337]
  ...
  F2 --> F3[Devoirs - création + date limite + clé publique prof]
  ...
  H1 --> H2[Comptes + clés privées + ETH fictif]
  ...
  O1 --> P[Documentation]
  P --> P1[README - install + sécurité + dépannage + tests]
...
subgraph U
  A[Admin - wallet deployer]
  P[Enseignant - wallet prof]
  E[Etudiant - wallet etudiant]
end

subgraph F
  UI[Frontend React]
  ETH[Ethersjs]
  CRYPTO[Chiffrement RSA et AES]
end

subgraph C
  SC[Smart Contract SystemeGestionControles]
end

subgraph B
  CHAIN[Hardhat Node 31337]
  ONCHAIN[(Donnees on chain)]
end

subgraph O
  API[Serveur upload Express]
  FILES[(Stockage fichiers chiffres)]
end
